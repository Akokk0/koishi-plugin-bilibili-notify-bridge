...
- **直连 BN，不走 koishi 的全局代理。** koishi 配了代理时，内网里的 BN 会被送进代理而连不上，接入 token 和一次性的取图地址也会跟着进代理。连 BN、下载 BN 给的图都直连。
- **图只取两种**：BN 自己的取图口（和桥接地址是同一台机器），以及公网上的图（图集是 B 站的原图）。指向内网或本机的其他地址一律不取，取回来的也必须真是图片，不然这条推送算失败、并说明原因。BN 挂在反向代理后面时，反代要设 `X-Forwarded-Proto`，不然 BN 给出的取图地址和桥接地址对不上。
